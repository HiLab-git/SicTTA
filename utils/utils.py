...
def add_gaussian_noise_3d(volume, mean=0, std=0.20):
        noise = torch.rand_like(volume)*std + mean
        noisy_volume = volume + noise
        return noisy_volume
def rotate_single_random(img):
    label = np.random.randint(0, 4, 1)[0]
    # label = 0
    img = add_gaussian_noise_3d(img)
    if label == 1:
...
def derotate_single_random(img,label):
    if label == 3:
        img = tensor_rot_90(img)
    elif label == 2:
        img = tensor_rot_180(img)
